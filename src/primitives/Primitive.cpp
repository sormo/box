...
#ifdef USE_CAMERA
#define CAMERA g_camera
#else
#include "..\MainActor.h"
#define CAMERA g_mainActor
#endif
...
namespace primitive
{
	...
	Primitive::~Primitive()
	{
		if (m_flags[(size_t)Flags::notZoomable] && g_mainActor)
			g_mainActor->unregisterOnScaleChange(this);
	}
	...
	void Primitive::doRender(const oxygine::RenderState& rs)
	{
		...
		if (m_lineColor.argb != 0)
		{
			...
			if (m_flags[(size_t)Flags::onMainActor] && !m_flags[(size_t)Flags::notZoomable])
				width = m_lineWidth / g_mainActor->getScaleCamera();
	...
	void Primitive::SetNotZoomable(bool set)
	{
		...
		if (set)
			g_mainActor->registerOnScaleChange(this, [this](float s) { setScale(1.0f/s); });
		else
			g_mainActor->unregisterOnScaleChange(this);
	...
	void Primitive::onAdded2Stage()
	{
		...
		oxygine::Actor * mainActorParent = dynamic_cast<oxygine::Actor*>(g_mainActor.get());
